...
DB_NAME="dbraporsp25"
U_USER="postgres"
U_PASS="rsp_2025dev_rebornrapor25#"
...
BEGIN
   IF NOT EXISTS (
      SELECT FROM pg_catalog.pg_roles WHERE rolname = '$U_USER'
   ) THEN
      CREATE ROLE $U_USER LOGIN PASSWORD '$U_PASS';
   END IF;
END
\$do\$;
ALTER ROLE $U_USER WITH PASSWORD '$U_PASS';
CREATE DATABASE $DB_NAME OWNER $U_USER;
\c $DB_NAME; 
\i /tmp/dbraporsp25/dberaporsplinux.sql;
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO $U_USER;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO $U_USER;
GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA public TO $U_USER;
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA nilai TO $U_USER;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA nilai TO $U_USER;
GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA nilai TO $U_USER;
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA pkl TO $U_USER;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA pkl TO $U_USER;
GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA pkl TO $U_USER;
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA projec TO $U_USER;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA projec TO $U_USER;
GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA projec TO $U_USER;
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA tambah TO $U_USER;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA tambah TO $U_USER;
GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA tambah TO $U_USER;
\q
EOF
